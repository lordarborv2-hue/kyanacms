...
let paymongoConfig = { enabled: false, rate: 100, public_key: '' };
...
// ─── EXC LABEL DEFINITIONS ──────────────────────────────────────────────────
// Maps item category (ItemType) to the 6 excellent option labels.
// Weapons: type 0-5  |  Sets/Armor: type 6-11  |  Wings: type 12
const excLabels = {
    weapon: [
        { value: 1,  label: 'Exc 1 HP Hunt' },
        { value: 2,  label: 'Exc 2 Mana Hunt' },
        { value: 4,  label: 'Exc 3 Dmg 2%' },
        { value: 8,  label: 'Exc 4 Speed' },
        { value: 16, label: 'Exc 5 EDR' },
        { value: 32, label: 'Exc 6 Dmg Lvl/20' },
    ],
    set: [
        { value: 1,  label: 'Exc 1 Zen' },
        { value: 2,  label: 'Exc 2 DSR' },
        { value: 4,  label: 'Exc 3 DD' },
        { value: 8,  label: 'Exc 4 Ref' },
        { value: 16, label: 'Exc 5 Increase Life' },
        { value: 32, label: 'Exc 6 Increase Mana' },
    ],
    wings: [
        { value: 1,  label: 'Exc 1 Ignore' },
        { value: 2,  label: 'Exc 2 Return' },
        { value: 4,  label: 'Exc 3 —' },
        { value: 8,  label: 'Exc 4 —' },
        { value: 16, label: 'Exc 5 HP Recov' },
        { value: 32, label: 'Exc 6 Mana Recon' },
    ],
};

function getExcGroup(itemType) {
    if (itemType >= 0  && itemType <= 5)  return 'weapon';
    if (itemType >= 6  && itemType <= 11) return 'set';
    if (itemType === 12)                  return 'wings';
    return 'weapon'; // fallback
}

/**
 * Rewrites the text of all 6 exc checkboxes to match the item type.
 * Only updates labels — preserves checked state and event handlers.
 */
function updateExcLabels(itemType) {
    const group  = getExcGroup(itemType);
    const labels = excLabels[group];
    const checks = document.querySelectorAll('.shop-exc');
    checks.forEach((cb, i) => {
        if (labels[i]) {
            // The checkbox is wrapped in a <label> element
            const lbl = cb.parentElement;
            // Replace text node while keeping the checkbox intact
            ...
            lbl.appendChild(document.createTextNode(' ' + labels[i].label));
            cb.value = labels[i].value;
        }
    });
}
...
async function loadUserData() {
    try {
        ...
        paymongoConfig = data.paymongo || { enabled: false, rate: 100, public_key: '' };
        ...
        const paymongoContainer = document.getElementById('paymongo-container');
...
function applyItemFilters() {
    ...
    if(opt.dataset.allowExc == 0) {
        document.querySelectorAll('.shop-exc').forEach(cb => cb.checked = false);
    } else {
        // ── UPDATE EXC LABELS based on item category ──────────────────────
        updateExcLabels(parseInt(opt.value.split('-')[0]));
    }
...
async function payWithPayMongo() {
    ...
    try {
        ...
        if (result.success) {
            ...
            if (paymentWindow) paymentWindow.location.href = result.checkout_url;
...
function updateQRPrice() {
    ...
    priceDisplay.textContent = (credits / ratio).toFixed(2);
}
...
async function sendQRProof() {
    ...
    try {
        const response = await fetch('Configuration/submit-qr-donation.php', { method: 'POST', body: formData });
...
function addToCart() {
    ...
    // Snapshot the selected exc names AT ADD TIME (not from live DOM later)
    ...
    // Snapshot harmony name at add time
    const harmonySelect = document.getElementById('shop-harmony');
    const harmonyName = harmonySelect.options[harmonySelect.selectedIndex].text;
    ...
    const cartItem = {
        ...
        harmonyName: harmonyName,  // ← snapshotted at add time
    ...
    if (cartDropdown && cartDropdown.style.display === 'none') toggleCart();
...
function showItemTooltip(e, uniqueId) {
    ...
    // ── FIX: use harmonyName (snapshotted at add-to-cart time) ──
